...
export default function PartsPage() {
  ...
  const fetchParts = useCallback(
    async () => {
      ...
      console.log(`[Shop] fetchParts called — page=${page}, skip=${skip}`);
      setLoading(true);
      setError(null);
      try {
        ...
        console.log(`[Shop] response — returned ${data.data.length} items, meta:`, data.meta);
        console.log(`[Shop] first item id: ${data.data[0]?.id}, title: ${data.data[0]?.translations[0]?.title}`);
        setParts(data.data);
        setMeta(data.meta);
      } catch (err) {
        console.error("[Shop] Error fetching parts:", err);
  ...
  useEffect(() => {
    console.log(`[Shop] useEffect triggered — page=${page}`);
    fetchParts();
  ...
  const getTitle = (part: Part): string => {
    ...
    return (
      part.translations.find((tr) => tr.language === localeStr)?.title ||
  ...
  if (loading) {
    return (
      <div
        className={`min-h-[70vh] w-full bg-zinc-950 ${oswald.variable} ${dmSans.variable}`}
      >
        {/* Header skeleton */}
  ...
  if (error) {
    return (
      <div
        className={`flex min-h-[70vh] items-center justify-center bg-zinc-950 px-4 ${oswald.variable} ${dmSans.variable}`}
      >
          ...
          <div className="grid grid-cols-2 gap-4 bg-zinc-950 px-6 py-6 sm:grid-cols-3 sm:gap-5 lg:grid-cols-4 xl:grid-cols-5">
            {parts.map((part) => {
              ...
              const initialPrice = part.initialPrice
                ...
                : null;
              const isOnSale =
                initialPrice !== null && initialPrice > price;
              const isSoldOut = part.quantity === 0;

              return (
                <Link
                  key={part.id}
                  href={`/${localeStr}/item/${slug}`}
                  className="group relative flex flex-col bg-zinc-900 border border-zinc-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-amber-500 hover:border-amber-500/50 transition-colors duration-200"
                >
                  {/* Image */}
                  <div className="relative aspect-square w-full overflow-hidden bg-zinc-900">
                    ...
                    <div className="absolute left-2 top-2 flex flex-col gap-1">
                      {isOnSale && (
                        ...
                        >
                          {Math.round(
                            ((initialPrice! - price) / initialPrice!) * 100,
                          )}
                          % OFF
                        </span>
                      )}
